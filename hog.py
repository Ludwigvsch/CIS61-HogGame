...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls==0:
        opponent_score= 1+max(opponent_score//10,opponent_score%10)
...
def select_dice(score, opponent_score):
    ...
    sum_bothscore =score + opponent_score
    if (sum_bothscore%7==0):
        return four_sided
    else:
        return six_sided
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    while num_dice < 11:
        ...
        print( str(num_dice) + " dice scores " + str(average) + " on average")
...
def swap_strategy(score, opponent_score):
    ...
    if bacon + score == opponent_score/2:
        return 0
    elif bacon + score == opponent_score * 2: 
        return BASELINE_NUM_ROLLS
    else:
...
def final_strategy(score, opponent_score):
    ...
    The final strategy is there to use the other strategies and rules in order
    to give the player the best results. It uses the swap strategy 
    when it is most beneficial, such as when the opponent has more 
    points. It returns zero in order to use free bacon if the player has 
    more points. It rolls five dice to give the player the max points.
    """
